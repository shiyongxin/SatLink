...
def sp_link_performance():  # this function runs the availability for a single point and shows a complete output
    ...
    path = os.path.join(script_dir, 'temp', 'args.pkl')
    ...
    path = os.path.join(script_dir, 'models', 'Modulation_dB.csv')
...
def mp_link_performance():
    ...
    path = os.path.join(directory, 'results ' + datetime.datetime.now().strftime('%y-%m-%d_%H-%M-%S') + '.csv')
